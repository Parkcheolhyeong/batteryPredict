...
data = pd.read_excel('K-WATER_total.xlsx', sheet_name = select_day )
df = pd.DataFrame(data, columns = ['rank', 'load', 'smp', 'fuel_cell', 'coal', 'solar', 'wind', 'hydraulic', 'ocean', 'bio', 'waste', 'LNG', 'supply', 'temp', 'rainfall', 'wind', 'humidity', 'cloud', 'discomfor_index', 'wind_temp', 'nuclear', 'b_coal', 'arthracite', 'gas', 'a_fluid', 'ex_smp', 'ex_load'] )
...
# load dataset
dataset = pd.read_excel('K-WATER_total.xlsx', sheet_name = select_day)
#print(dataset)
...
dataset.to_excel(select_predict + '_'+ select_day + '.xlsx')
